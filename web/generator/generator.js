...
const QUESTIONS_STRINGS = {
  ...
  containerType: 'Do You want a Page Container?',
}
...
const QUESTIONS = [
  ...
  {
    name: QUESTIONS_STRINGS.containerType,
    type: 'confirm',
  },
  {
...
const renderContainerType = (isPage) => {
  return isPage ? 'Page' : 'Container'
}

inquirer.prompt(QUESTIONS).then((answers) => {
  const containerName = answers[QUESTIONS_STRINGS.containerName]
  const containerType = answers[QUESTIONS_STRINGS.containerType]
  ...
  const newFilesPath = `${process.cwd()}/src/containers/${
    containerName + renderContainerType(containerType)
  }`
  ...
  filesToCreate.forEach((file) => {
    ...
    if (stats.isFile()) {
      ...
      if (withActions && filesWithActions.includes(file)) {
        console.log(
          'actions templateFilePathWithActions: ' + templateFilePathWithActions,
        )
